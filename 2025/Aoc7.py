...
def partTwo(lines):
    ...
    for line in lines[1:]:
        ...
        for beamIndex, count in beamCounts.items():
            if line[beamIndex] == "^":
                newBeams[beamIndex + 1] += count
                newBeams[beamIndex - 1] += count
            else:
                newBeams[beamIndex] += count
        beamCounts = newBeams
    return sum(beamCounts.values())
...
"""
    ...
    assert partOneResult == 21, "got result: {}".format(partOneResult)

    partTwoResult = partTwo(testlines)
    assert partTwoResult == 40, "got result: {}".format(partTwoResult)
